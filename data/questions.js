...
,
  {"id": 313, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Để TRACE NGƯỢC một lô hàng có nguy cơ nhiễm khuẩn từ thành phẩm về vendor gốc trong BC, cần kết hợp các tool/setup nào? (Chọn nhiều)", "options": ["Item Tracking Code với Lot Specific Tracking bật", "Item Tracing tool với Trace Method = Usage → Origin", "Tạo thêm Variant Code cho mỗi lô", "Item Ledger Entries filter Lot No.", "Document Attachment trên Lot No. Information Card (cho CoA, C/O)", "Cấu hình Bin Mandatory ở Location"], "correct": [0, 1, 3, 4], "exp": ["A đúng: Lot Specific Tracking là điều kiện cần để có chain trace.", "B đúng: Item Tracing là tool chính, Trace Method quyết định hướng trace.", "C SAI: Variant Code cho biến thể (size/colour), không phải tracking.", "D đúng: ILE filter lot là source of truth cho mọi giao dịch của lô.", "E đúng: Lot Info Card lưu CoA/C/O thiết yếu cho audit trace.", "F SAI: Bin Mandatory liên quan put-away, không cần thiết cho trace."]},
  {"id": 314, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 5, "q": "Đang setup Manufacturing trong BC cho khách FMCG. Các yếu tố nào CẦN có để Backflushing chính xác? (Chọn nhiều)", "options": ["Flushing Method = Backward trên Item Card của component", "Routing Link Code đúng giữa BOM line và Operation", "Item Tracking Code không bật (để tránh phức tạp)", "Production BOM ở status Certified", "Routing ở status Certified", "Adjust Cost — Item Entries chạy mỗi đêm", "Item phải có Costing Method = Standard (bắt buộc)"], "correct": [0, 1, 3, 4, 5], "exp": ["A đúng: Flushing Method = Backward thì component mới auto-consume khi post Output.", "B đúng: Không có Routing Link Code đúng → consume sai operation → tồn kho lệch.", "C SAI: Item Tracking Code phải bật cho food (theo dõi lô + hạn dùng). Không liên quan đến backflush ổn định.", "D đúng: BOM phải Certified mới được dùng trong Production Order.", "E đúng: Routing tương tự.", "F đúng: Adjust Cost phải chạy định kỳ để cost của component được cập nhật chính xác trước backflush.", "G SAI: Costing method nào cũng được (Standard/Average/FIFO/Specific). Backflush không phụ thuộc."]},
  {"id": 315, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Đóng kỳ kế toán cuối tháng trong BC — các job/việc PHẢI hoàn thành TRƯỚC khi lock fiscal period. (Chọn nhiều)", "options": ["Post tất cả Sales Invoice, Purchase Invoice của kỳ", "Chạy Adjust Cost — Item Entries", "Chạy Post Inventory Cost to G/L", "Chạy Adjust Exchange Rates (nếu có ngoại tệ)", "Reconcile AR/AP/Bank/Inventory với G/L Trial Balance", "Run Close Income Statement", "Sign-off CFO trên Reconciliation Report"], "correct": [0, 1, 2, 3, 4, 6], "exp": ["A đúng: Mọi invoice phải post — không skip.", "B đúng: Adjust Cost trước Post Inventory Cost to G/L.", "C đúng: Post Inventory Cost to G/L sync sub-ledger ↔ G/L.", "D đúng: Adjust Exchange Rates revalue ngoại tệ.", "E đúng: Reconciliation phải khớp 100% trước close.", "F SAI: Close Income Statement chỉ chạy 1 lần/năm sau audit ký BCTC, KHÔNG phải mỗi tháng. Đây là bẫy.", "G đúng: Sign-off CFO bắt buộc — là life insurance khi audit."]},
  {"id": 316, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "7 Wastes (Muda) trong Lean Manufacturing — các mục nào THỰC SỰ nằm trong danh sách waste theo TPS? (Chọn nhiều)", "options": ["Transportation (di chuyển không cần thiết)", "Inventory (tồn kho dư)", "Motion (cử động thừa)", "Customer Satisfaction (sự hài lòng khách)", "Waiting (chờ đợi)", "Over-production (sản xuất thừa)", "Over-processing (xử lý thừa)", "Defects (lỗi sản phẩm)", "Capital Investment (đầu tư vốn)"], "correct": [0, 1, 2, 4, 5, 6, 7], "exp": ["A, B, C, E, F, G, H đều đúng — là 7 wastes chính (TIM-WOOD: Transportation, Inventory, Motion, Waiting, Over-production, Over-processing, Defects).", "D SAI: Customer Satisfaction là MỤC TIÊU, không phải waste.", "I SAI: Capital Investment là chi phí kinh doanh, không phải waste theo định nghĩa Lean.", "Bonus: thứ 8 (Skills underutilization) đôi khi được thêm vào — không có trong 7 gốc nhưng được nhiều tổ chức sử dụng."]},
  {"id": 317, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 5, "q": "Phân biệt MES (Manufacturing Execution System) vs ERP — chức năng nào THUỘC về MES, không phải ERP? (Chọn nhiều)", "options": ["Tracking real-time trạng thái từng máy + từng operator", "Hoạch định nhu cầu vật tư (MRP)", "Andon — cảnh báo trực quan tại trạm", "Tính OEE (Availability × Performance × Quality) real-time", "Posting Sales Invoice + sinh AR", "Quality test result capture tại shop floor", "Period-end accounting close"], "correct": [0, 2, 3, 5], "exp": ["A đúng: Real-time tracking là chức năng cốt lõi MES.", "B SAI: MRP là chức năng ERP (Level 4 ISA-95).", "C đúng: Andon là MES (visual management at shop floor).", "D đúng: OEE real-time cần data từ sensor/PLC → MES tổng hợp.", "E SAI: AR posting là ERP.", "F đúng: QC capture tại shop floor là MES.", "G SAI: Period-end close là ERP/Finance."]},
  {"id": 318, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Mock recall drill (drill thu hồi giả định) cho ngành food là quan trọng. Mock recall thành công đo bằng các tiêu chí nào? (Chọn nhiều)", "options": ["Trace ngược toàn bộ giao dịch của 1 lô trong dưới 2 giờ", "Tìm được 100% khách hàng đã nhận lô", "Tăng doanh số quý đó", "Liên hệ và confirm receipt từ khách trong 24 giờ", "Thu hồi vật lý ≥99% lô đã bán", "Document toàn bộ quá trình cho audit (FDA / Bộ Y tế)", "Giảm chi phí marketing"], "correct": [0, 1, 3, 4, 5], "exp": ["A đúng: 2h là benchmark FDA cho food.", "B đúng: Phải có FULL customer list — sót 1 khách = sót 1 nguy cơ.", "C SAI: Doanh số không liên quan recall.", "D đúng: 24h response time là chuẩn ngành.", "E đúng: ≥99% recovery rate là target.", "F đúng: Document audit trail là bắt buộc pháp lý.", "G SAI: Marketing cost không phải tiêu chí mock recall."]},
  {"id": 319, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "POS hiện đại trong Retail có những đặc điểm nào? Đặc biệt với Mobile/Wireless POS. (Chọn nhiều)", "options": ["Cho phép line-busting (giảm hàng chờ tại quầy)", "Bắt buộc cài application local trên thiết bị (Thick Client)", "Có thể chấp nhận thanh toán tại bàn (trong nhà hàng)", "Yêu cầu wireless network infrastructure mạnh", "Chỉ chạy được trên iPad", "Tích hợp với MMS để cập nhật tồn kho tức thì", "Phụ thuộc 100% kết nối với server trung tâm (Thin Client)"], "correct": [0, 2, 3, 5], "exp": ["A đúng: Line-busting là use case classic của Mobile POS (Apple Store).", "B SAI: Mobile POS có thể Thin hoặc Thick — không bắt buộc.", "C đúng: Tablet POS tại bàn là common.", "D đúng: Wireless infra mạnh là điều kiện tiên quyết và là cost ban đầu cao nhất.", "E SAI: Mobile POS chạy được trên Android/Windows.", "F đúng: Integration với MMS = cốt lõi (POS feed sale → MMS tính tồn).", "G SAI: Chỉ Thin Client mới phụ thuộc 100% — Thick có thể offline."]},
  {"id": 320, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Customer Relationship Management (CRM) trong Retail — 4 giai đoạn trưởng thành (maturity) gồm cái nào? (Chọn nhiều)", "options": ["Operational CRM (Frequent Shopper Programs)", "Marketing CRM (segmentation cơ bản)", "Financial CRM (tính lãi/lỗ khách hàng)", "Analytical CRM (Customer Profiling với 3-6 segments)", "Strategic CRM (Lifestyle Portrait + Micro-marketing)", "Technical CRM (chỉ về phần mềm)"], "correct": [0, 1, 3, 4], "exp": ["A đúng: Giai đoạn 1 — thu thập data cơ bản qua loyalty program.", "B đúng: Giai đoạn 2 — segmentation + mass marketing.", "C SAI: \"Financial CRM\" không phải khái niệm chuẩn.", "D đúng: Giai đoạn 3 — Analytical với customer profiling.", "E đúng: Giai đoạn 4 — Strategic, deep personalization.", "F SAI: CRM không phải vấn đề công nghệ — là chiến lược kinh doanh."]},
  {"id": 321, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Workshop ERP Consulting đạt mức L3 (Đạt) trong nhóm 'Kết quả & Decisions' phải có gì? (Chọn nhiều)", "options": ["Mỗi sub-process được decide rõ (To-Be flow approved)", "Action items đầy đủ owner + deadline", "Demo thật ấn tượng để khách bị choáng ngợp", "Gaps được phân loại (in-scope / customization / process change / out-of-scope)", "Sign-off của decision maker (CFO / CEO / CIO)", "Bữa ăn trưa miễn phí cho khách", "MoM gửi trong 24-48h"], "correct": [0, 1, 3, 4, 6], "exp": ["A, B, D, E, G đều là tiêu chí L3 chính thức cho 'Kết quả + ra quyết định' và 'Tài liệu & follow-up'.", "C SAI: Demo ấn tượng là chiêu trò, không phải outcome chuẩn.", "F SAI: Bữa ăn không phải tiêu chí workshop."]},
  {"id": 322, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Consultant fresher (L1) làm workshop bị overrun thời gian, khách bắt đầu mất kiên nhẫn. Phản ứng nào là L4 (Xuất sắc)? (Chọn nhiều)", "options": ["Acknowledge thời gian: 'Em xin phép tóm tắt phần này nhanh để vào chủ đề chính'", "Tiếp tục như planned, không đề cập thời gian", "Hỏi khách: 'Anh chị muốn em deep-dive phần nào nhất trong 30 phút còn lại?'", "Park các topic đi xa scope vào 'follow-up'", "Đổ lỗi cho team consultant khác chuẩn bị chậm", "Đề xuất re-schedule 1 workshop follow-up cho phần còn thiếu", "Cố gắng cover hết bằng cách nói nhanh hơn"], "correct": [0, 2, 3, 5], "exp": ["A đúng: Acknowledge thời gian + acknowledge khách = L4.", "B SAI: Ignore vấn đề = L1.", "C đúng: Đặt câu hỏi mở để khách reprioritize = L4 facilitation.", "D đúng: Parking lot là tool L3+ chuyên nghiệp.", "E SAI: Đổ lỗi = mất uy tín ngay.", "F đúng: Đề xuất follow-up = give khách option, L4.", "G SAI: Nói nhanh = khách stress thêm."]},
  {"id": 323, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Pre-sales BC project — các 'deal-breaker' nào CẦN check NGAY trong cuộc gặp đầu tiên với khách Việt Nam? (Chọn nhiều)", "options": ["Khả năng tích hợp e-invoice (NĐ 123/2020, NĐ 70/2025)", "Catch weight (nếu khách là thịt/hải sản) — BC core KHÔNG hỗ trợ", "Process Manufacturing với recipe — cần ISV không phải BC core", "Khách có ưa thích màu xanh hay đỏ", "EDI cho MT (modern trade)", "Van Sales / DMS cho >20 xe — cần ISV", "Mức lương trung bình của khách"], "correct": [0, 1, 2, 4, 5], "exp": ["A đúng: E-invoice là pháp lý, không có không bán hàng được.", "B đúng: Catch weight là kiến trúc BC không hỗ trợ — phải đề xuất ISV ngay pre-sales.", "C đúng: Process Mfg cần ISV chuyên ngành.", "D SAI: Màu sắc không phải deal-breaker.", "E đúng: EDI cho MT cần ISV.", "F đúng: Van Sales >20 xe cần ISV (Naviworld FMCG, Sana FMCG).", "G SAI: Lương khách không liên quan."]},
  {"id": 324, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Data migration trong dự án BC — các bước CẦN có trước khi cut-over go-live. (Chọn nhiều)", "options": ["Data profiling lần đầu để discover dirty data", "Khách cleansing data theo report issues", "Mock migration tối thiểu 3 lần", "Tự động chuyển hết data cũ vào BC, không quan tâm chất lượng", "Cut-off date với GIỜ cụ thể", "Reconciliation report khớp 100% với hệ thống cũ", "Sign-off của CFO trên Reconciliation", "Open AR/AP > 365 ngày phải write off trước cut-off"], "correct": [0, 1, 2, 4, 5, 6, 7], "exp": ["A đúng: Data profiling phải làm đầu tiên.", "B đúng: Khách clean (họ hiểu business meaning).", "C đúng: 3 mock là MINIMUM theo best practice.", "D SAI: 'Tự động chuyển hết' = garbage in garbage out.", "E đúng: Cut-off phải có timezone + giờ cụ thể.", "F đúng: Reconciliation 100% là điều kiện cut-over.", "G đúng: Sign-off CFO bắt buộc.", "H đúng: Bad debt > 1 năm không nên migrate."]},
  {"id": 325, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Customization trong BC — các cách thay thế CUSTOM AL code mà consultant L4 sẽ ưu tiên trước. (Chọn nhiều)", "options": ["Configuration trong BC core (Setup pages)", "Process change (khách đổi cách làm theo best practice)", "Power Automate flow cho approval/notification logic", "Custom AL extension viết từ đầu", "ISV solution đã được validate trên AppSource", "Power BI report thay vì custom RDL report", "Tự build database riêng bên ngoài BC"], "correct": [0, 1, 2, 4, 5], "exp": ["Theo quy tắc CCP (Config → Customize → Process change → Live with it):", "A đúng: Config là first preference.", "B đúng: Process change tốt hơn custom (giảm tech debt).", "C đúng: Power Automate cho workflow là phương án no-code.", "D KHÔNG ưu tiên: AL custom = last resort.", "E đúng: AppSource ISV validated > custom build.", "F đúng: Power BI > custom RDL (dễ maintain, drill-down).", "G SAI: Database riêng = sai kiến trúc, integration nightmare."]},
  {"id": 326, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Reconciliation report cuối tháng phải khớp giữa các domain — domain nào phải khớp với G/L Trial Balance? (Chọn nhiều)", "options": ["Customer Age (AR sub-ledger)", "Vendor Age (AP sub-ledger)", "Bank Account Reconciliation", "Inventory Valuation Report", "Employee Master List", "Open Sales Order Report", "Posted Sales Invoice List"], "correct": [0, 1, 2, 3], "exp": ["A đúng: AR Aging tổng phải = G/L AR balance.", "B đúng: AP Aging tổng phải = G/L AP balance.", "C đúng: Bank Rec phải = G/L Bank account.", "D đúng: Inventory Value tổng = G/L Inventory.", "E SAI: Employee list là master data, không có G/L tương ứng.", "F SAI: Open SO là sales pipeline, không hit G/L.", "G SAI: Posted Invoice đã hit G/L rồi — không cần khớp riêng, mà là source."]},
  {"id": 327, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Cost of Goods Manufactured (COGM) — các thành phần nào tạo nên COGM trong sản xuất? (Chọn nhiều)", "options": ["Direct Material (nguyên liệu trực tiếp)", "Direct Labor (lương công nhân trực tiếp)", "Manufacturing Overhead (điện, khấu hao máy, lương quản lý xưởng)", "Marketing Cost (chi phí marketing)", "Beginning WIP - Ending WIP", "Sales commission", "R&D Cost"], "correct": [0, 1, 2, 4], "exp": ["Công thức: COGM = Beginning WIP + (DM + DL + MOH) − Ending WIP", "A, B, C đúng: 3 thành phần cost của sản xuất.", "D SAI: Marketing là SG&A, không vào COGM.", "E đúng: WIP change được tính trong COGM.", "F SAI: Sales commission là SG&A.", "G SAI: R&D là OpEx, capitalized hoặc expense, không vào COGM."]},
  {"id": 328, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Spaced Repetition System (SRS) hiệu quả — các nguyên tắc nào ĐÚNG? (Chọn nhiều)", "options": ["Interval tăng theo cấp số nhân khi đúng (1d, 3d, 7d, 14d, 30d)", "Khi sai → reset về 1 ngày", "Confidence rating (Tự tin/Phân vân/Đoán) ảnh hưởng schedule", "Ease factor không thay đổi theo thời gian", "Câu nào sai nhiều lần → ease factor giảm, lặp dày hơn", "Học liên tục 10 giờ/ngày sẽ hiệu quả hơn 30 phút mỗi ngày", "Retrieval Practice (kéo từ trí nhớ) hiệu quả hơn re-reading"], "correct": [0, 1, 2, 4, 6], "exp": ["A đúng: Cấp số nhân là core của SM-2 algorithm.", "B đúng: Sai → reset interval, ease giảm.", "C đúng: Confidence quyết định quality rating (Sure=Easy, Doubt=Good, Guess=Hard).", "D SAI: Ease factor THAY ĐỔI theo từng lần rate (giảm khi sai/khó, tăng khi dễ).", "E đúng: Lapses tracking + ease giảm → interval ngắn hơn.", "F SAI: Spaced repetition (giãn cách) tốt hơn massed practice (10h liền).", "G đúng: Retrieval practice là tâm điểm của SRS — kéo từ trí nhớ hiệu quả hơn đọc lại."]},
  {"id": 329, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Để phát hiện 'blind-spot' (điểm mù — tự tin sai) trong học tập, các metric nào hữu ích? (Chọn nhiều)", "options": ["Confidence \"Tự tin\" nhưng sai (overconfidence)", "First-try accuracy thấp dù overall accuracy cao", "Hard accuracy (★★★★+) thấp", "Streak ngày học cao", "Số câu đã trả lời", "Phân bố confidence so với correctness theo nhóm"], "correct": [0, 1, 2, 5], "exp": ["A đúng: Sure-but-wrong là dunning-kruger điển hình.", "B đúng: First-try accuracy thấp = học vẹt qua retry.", "C đúng: Hard accuracy thấp = không hiểu sâu, chỉ recall.", "D SAI: Streak chứng tỏ kỷ luật, không phải blind-spot detection.", "E SAI: Volume không phản ánh chất lượng.", "F đúng: Calibration plot (confidence vs accuracy) là phương pháp chuẩn để đo blind-spot."]},
  {"id": 330, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Bạn là Senior consultant mentor junior. Các kỹ năng quan trọng nhất cần truyền lại cho junior. (Chọn nhiều)", "options": ["Cách nói 'Em sẽ verify và confirm trong 24h' thay vì trả lời liều", "Stakeholder mapping (Power × Interest matrix)", "Cách dùng câu hỏi mở (Open-ended) trong workshop", "Cách dìm các consultant đối thủ", "Documentation discipline (MoM trong 24h, Decision Log)", "Quy tắc CCP (Configure > Customize > Process change > Live with)", "Bí quyết trốn responsibility khi project khó khăn", "Cách phân biệt 'It works in DEV' vs production reality"], "correct": [0, 1, 2, 4, 5, 7], "exp": ["A đúng: Kỹ năng quản lý uncertainty, không over-commit.", "B đúng: Stakeholder mapping là baseline skill.", "C đúng: Open question là kỹ thuật facilitation cốt lõi.", "D SAI: Anti-pattern — không ai mentor cái này.", "E đúng: Documentation = life insurance.", "F đúng: Quy tắc CCP là kim chỉ nam.", "G SAI: Trốn responsibility = sa thải.", "H đúng: Production-grade thinking là khác biệt junior vs senior."]},
  {"id": 331, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Multi-tenant cloud BC — các yêu cầu nào CẦN có cho custom code (AL extension) để chạy ổn định trên BC Online? (Chọn nhiều)", "options": ["Universal Code compliance (không dùng DotNet, không lock object)", "Per-Tenant Extension (PTE) deploy qua Admin Center", "Hoặc AppSource validated app", "Không được dùng custom database table riêng bên ngoài BC", "Phải có Telemetry/logging (App Insights)", "Cài đặt trực tiếp trên server SQL Database", "Phải hỗ trợ cả OnPrem và Cloud"], "correct": [0, 1, 2, 3, 4], "exp": ["A đúng: Universal Code là REQUIREMENT từ BC v22.", "B đúng: PTE là cách deploy custom code không qua AppSource.", "C đúng: AppSource là alternative cho code chia sẻ nhiều tenant.", "D đúng: Multi-tenant không có direct DB access — phải dùng AL pattern.", "E đúng: Telemetry là best practice cho production code.", "F SAI: SQL Database direct access KHÔNG được phép trên BC Online.", "G SAI: Code có thể chỉ target Cloud, không bắt buộc OnPrem."]},
  {"id": 332, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Power BI tích hợp với BC năm 2026 — các phương án integration. (Chọn nhiều)", "options": ["Microsoft Fabric Mirroring (Link to Microsoft Fabric) — option mới nhất", "OData V4 API endpoint từ BC", "BC REST API với OAuth 2", "Direct SQL connection vào BC database", "Power BI Embedded trong BC Role Center", "Excel export thủ công mỗi ngày", "FTP file dump"], "correct": [0, 1, 2, 4], "exp": ["A đúng: Fabric Link là phương án native mới nhất 2024+.", "B đúng: OData V4 cho live query.", "C đúng: REST API cho integration nâng cao.", "D SAI: Direct SQL không được phép trên BC Online (security/multi-tenancy).", "E đúng: Embed Power BI tile/report trong Role Center.", "F SAI: Manual = không scalable.", "G SAI: FTP file là legacy pattern, không phải integration."]},
  {"id": 333, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Sustainability module trong BC (từ 2024) hỗ trợ gì? Phù hợp scenario nào? (Chọn nhiều)", "options": ["Track CO2 emissions per Item/Vendor/Activity", "Sustainability Account hoạt động như Chart of Account cho ESG", "Tự động tính carbon tax phải nộp cho chính phủ", "Required cho EU CSRD compliance từ 2024", "Tính thay cho hệ thống ERP — không cần BC", "Posting Sustainability Ledger Entry song song với G/L", "Generate ESG report cho cổ đông"], "correct": [0, 1, 3, 5, 6], "exp": ["A đúng: Track theo Item/Vendor/Activity là core feature.", "B đúng: Sustainability Account mirror Chart of Account.", "C SAI: Không tự tính carbon tax — chỉ track emissions.", "D đúng: EU CSRD yêu cầu reporting từ 2024 (companies > 250 employees).", "E SAI: Sustainability là MODULE trong BC, không thay thế.", "F đúng: Sustainability Ledger Entry song song với G/L Entry.", "G đúng: ESG report cho cổ đông là use case chính."]},
  {"id": 334, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Cẩm nang triển khai — go-live thành công cần các yếu tố nào? (Chọn nhiều)", "options": ["Vendor onsite 5-7 ngày tuần đầu", "Hypercare 4-8 tuần (không 2 tuần)", "Manual procedure giấy backup cho 48h đầu", "Cut-over plan với rollback option", "Communication toàn organization 1 tuần / 1 ngày / 1 giờ trước", "Tránh go-live thứ Sáu hoặc cuối tháng", "Tổ chức party lớn ngay go-live day", "Train end users vào tuần go-live (last minute)"], "correct": [0, 1, 2, 3, 4, 5], "exp": ["A đúng: Onsite tuần đầu là critical, remote không bù được.", "B đúng: 2 tuần là không đủ — phát hiện edge case ở tuần 3-4.", "C đúng: Manual backup procedure là life insurance.", "D đúng: Rollback plan = breathing room.", "E đúng: Communication scheduled multi-touchpoint.", "F đúng: Thứ Hai đầu tháng tối ưu.", "G SAI: Party đẹp nhưng không phải success factor.", "H SAI: Train last minute = chaos. Train 2-4 tuần trước go-live."]},
  {"id": 335, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Politics trong ERP project — các pattern thực tế consultant L4 cần nhận diện. (Chọn nhiều)", "options": ["CFO thường là decision maker thật, không phải CEO", "IT Manager là ally, không phải đối thủ", "Champion ở phía khách > Senior consultant vendor", "Resistor (chống ERP) phải gặp riêng 1-1, không công khai", "Steering committee chỉ họp khi có vấn đề (reactive)", "Document mọi quyết định quan trọng qua email + signed PDF", "Đứng cùng phe vendor cố sell thêm scope cho khách"], "correct": [0, 1, 2, 3, 5], "exp": ["A đúng: CFO decide spec/vendor/timeline thực tế.", "B đúng: IT Manager maintain 10 năm tới — alliance.", "C đúng: Champion ở lại lâu hơn senior consultant.", "D đúng: 1-1 cho resistor — defensive khi công khai.", "E SAI: Steering committee phải họp REGULARLY, không reactive.", "F đúng: Documentation = life insurance khi politics nổ ra.", "G SAI: L4 đứng cùng phe KHÁCH, không phải vendor sell."]},
  {"id": 336, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★)", "difficulty": 4, "q": "Câu hỏi nào trong workshop fit-gap KHÔNG nên hỏi? (Câu này NGƯỢC — chọn các câu KHÔNG nên hỏi) (Chọn nhiều)", "options": ["'Nếu bắt đầu lại từ đầu, anh có làm vậy không?'", "'Quy trình này có ai biết tại sao tồn tại không?'", "'Khi có lỗi/exception thì xảy ra gì?'", "'Anh chị thấy hệ thống cũ tệ chỗ nào?' (mở đầu bằng câu này)", "'Anh muốn report giống Excel template kèm theo không?'", "'Tại sao team trước làm sai vậy?'", "'Hiện tại tháng cuối năm khác tháng thường thế nào?'"], "correct": [3, 4, 5], "exp": ["Đáp án là 3 câu KHÔNG NÊN hỏi:", "D KHÔNG nên: Mở đầu bằng tiêu cực = khách phòng thủ, mất trust.", "E KHÔNG nên: Hỏi 'giống Excel' = đóng khung tư duy, không cho phép best practice.", "F KHÔNG nên: 'Team trước làm sai' = chính trị, gây xung đột.", "A, B, C, G đều là câu hỏi tốt L3+: probe để hiểu rationale, exception cases."]},
  {"id": 337, "type": "multi", "section": "AB. Anti-vẹt — Multi Choice (★★★★★)", "difficulty": 5, "q": "Performance issue sau go-live: User báo 'BC chậm'. Các bước debug ĐÚNG. (Chọn nhiều)", "options": ["Yêu cầu user cung cấp specific operation, thời gian, screenshot", "Check Telemetry / App Insights cho slow query", "Mua server mạnh hơn ngay lập tức", "Kiểm tra database growth + largest tables", "Performance test với 3x production volume trong sandbox", "Đổ lỗi cho user dùng laptop yếu", "Run Diagnostic Tool / Profiler trên specific operation", "Disable mọi custom AL extension để test"], "correct": [0, 1, 3, 4, 6, 7], "exp": ["A đúng: Tránh subjective 'chậm' — cần data cụ thể.", "B đúng: Telemetry shows real bottleneck.", "C SAI: Mua server mạnh trước khi diagnose = lãng phí.", "D đúng: Database growth là common cause.", "E đúng: Replicate trong sandbox để test isolate.", "F SAI: Đổ lỗi user = không professional.", "G đúng: Profiler = scientific debugging.", "H đúng: Disable extension để bisect = standard technique."]}
];
